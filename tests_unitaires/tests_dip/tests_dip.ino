...
#define moteurGaucheAvant   10
#define moteurGaucheArriere 11
...
**/
bool getLigne(){
	if(!digitalRead(capteurLigneGauche)) return 2;
	if(!digitalRead(capteurLigneDroit))  return 1;
	return  0;
}
...
size_t getAdversaire(){
	...
	if(digitalRead(capteurGauche)) return 3;
	if(digitalRead(capteurDroit))  return 1;
	if(digitalRead(capteurAvant))  return 2;
	return 0;
...
size_t dipToInt(){
	...
	Serial.println(String("DIP1 : ") + digitalRead(DIP1) + "\t DIP2 : " + digitalRead(DIP2)+ "\t DIP3 : " + digitalRead(DIP3));
...
void routine(){
	while(true){
	    if(getLigne()){
	    	if(getLigne() == 1){ // on recule et on repart sur la gauche
	    		setMoteur(droit  , avant  , 128 );
	    		setMoteur(gauche , arriere, 128 );
	    		delay(200);
	    	}else{
	    		setMoteur(droit  , arriere, 128 );
	    		setMoteur(gauche , avant  , 128 );
	    		delay(200);
	    	}
	    }
...
void setup(){
	...
	pinMode(capteurLigneGauche, INPUT);
	pinMode(capteurLigneDroit, INPUT);
